...
async function getSummaryRenderData(){
  ...
  const phoneSummarySections=platforms.map((platform)=>buildPhoneSummarySection(platform,getOrdersForStatus(day.platforms[platform],"active"))).filter(Boolean).join("");
  return{dateKey,preparedBy:day.approval.preparedBy,checkedBy:day.approval.checkedBy,daySummary,summarySections:sections,phoneSummarySections};
}
